...
MAT     *mat;
{
     ...
     redo:
	  ...
	  for ( j=0; j<n; j++ )
	       ...
	       } while ( *line=='\0' || sscanf(line,"%llf",&mat->me[i][j])<1 );
#endif
...
#elif REAL == LONGDOUBLE
	       if ((io_code=fscanf(fp,"%llf",&mat->me[i][j])) < 1 )
#endif
...
#elif REAL == LONGDOUBLE
          } while ( *line=='\0' || sscanf(line,"%llf",&vec->ve[i]) < 1 );
#endif
...
#elif REAL == LONGDOUBLE
	  if ((io_code=fscanf(fp,"%llf",&vec->ve[i])) < 1 )
#endif
	       error(io_code==EOF ? 7 : 6,"bfin_vec");
