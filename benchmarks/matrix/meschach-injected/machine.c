...
/* __ip__ -- inner product with fp injected*/
double	__ip_fp__(dp1,dp2,len,pcounter)
register Real	*dp1, *dp2;
int	len;
int *pcounter;
{
    ...
    dp1 += 4*len4;	dp2 += 4*len4;
#endif
    Real src_mlt, src_ori, dest;
    for ( i = 0; i < len; i++ ){
	//sum  += dp1[i]*dp2[i];
	src_mlt  = dp1[i]*dp2[i];
	src_ori  = sum;
	dest = src_ori + src_mlt;
	sum = dest;

	uniform_sampled_fp_injection((void *)&src_ori, (void *)&src_mlt, (void *)&dest, pcounter, 2, ALL_FP);
    }
...
/* __mltadd__ -- scalar multiply and add c.f. v_mltadd() */
void	__mltadd_fp__(dp1,dp2,s,len,pcounter)
register Real	*dp1, *dp2;
register double s;
register int	len;
int * pcounter;
{
	...
	uniform_sampled_fp_injection((void *)&src_ori, (void *)&src_mlt, (void *)&dest, pcounter, 3, ALL_FP);
